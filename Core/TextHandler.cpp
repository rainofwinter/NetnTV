...
{
	return (int)ceil((float)pointSize / 72 * 96);
}
...
StringData::~StringData()
{
	...
	BOOST_FOREACH(StringDataElement & data, renderData)
	{
...
void TextHandler::setMagnification(float mag){

	font_.setTextureScaleSize(mag);
}

float TextHandler::getMagnification(){

	return font_.getTextureScaleSize();
}

void TextHandler::drawString(GfxRenderer * gl, const StringData * str,
...
{	
	...
	if(font_.shadow){
		...
		float shift_trans = font_.pointSize * 0.035;
		...
		float transform_[] =
		{
			...
			0, 1, 0, 0,
			0, 0, 1, 0,
			shift_trans, shift_trans, 0, 1
		};
	

		gl->useConvProgram();
		gl->applyCurrentShaderMatrix();
		gl->setConvProgramOpacity(opacity);	
		gl->setConvProgramSize((textureDim_ * 2.0f) / (Global::instance().curSceneMagnification()));
		gl->setConvProgramType(1); // GausBlur

		BOOST_FOREACH(const StringDataElement & data, str->renderData)
		{
			gl->use(data.texture);
...
// 		gl->applyCurrentShaderMatrix();
// 		gl->setConvProgramOpacity(opacity);	
// 		gl->setConvProgramSize(textureDim_ * 4.0f);
// 		gl->setConvProgramType(0); // Sharpness
...
void TextHandler::genChar(wchar_t c, unsigned int index)
{
	if (!init_) return;
	CharData charData;
...
// 		fontsize = 32;
	...
	FT_Set_Char_Size(ftFace_, fontsize * 64 , fontsize * 64, 96, 96);
	...
	if(!ftFace_->charmap && ftFace_->num_charmaps > 0){

		encoding = ftFace_->charmaps[0]->encoding;

		err = FT_Select_Charmap(ftFace_, encoding);
	}else if(ftFace_->charmap){
		encoding = ftFace_->charmap->encoding;
	}


	if(c != 0x20 && c >= 0xAC00 && c <= 0xD7A3 && encoding == FT_ENCODING_WANSUNG){ //KS5601
		code = unicode2ksc(c);
	}else
		code = c;


	FT_UInt glyphIndex = FT_Get_Char_Index(ftFace_, code);
	
	const CharData *pBaseCharData = NULL;
	TextHandler *baseTextHandler = NULL;

	if(glyphIndex == 0 && c != 0x0A && font_.fontFile != Text_2::getBaseFontFilePath()){
		TextFont baseFont = font_;
		baseFont.fontFile = Text_2::getBaseFontFilePath();
		baseFont.setTextureScaleSize(Global::instance().curSceneMagnification());

		baseTextHandler = Global::instance().curDocument()->renderer()->textHandler(baseFont);

		if(baseTextHandler->isinit()){
			
			std::wstring wstr;

			wstr.push_back(c);

			baseTextHandler->genAnyNewChars(wstr);

			pBaseCharData = baseTextHandler->charData(c);

			charData.index = index;

			charData.texWidth = pBaseCharData->texWidth;
			charData.texHeight = pBaseCharData->texHeight;

			const unsigned char *pBaseFontBuffer = baseTextHandler->imgBuffer();

			int baseLocalCharIndex = pBaseCharData->index % baseTextHandler->numCharsPerTexture();
			int baseCharXIndex = baseLocalCharIndex % baseTextHandler->numCharsPerTextureRow();
			int baseCharYIndex = baseLocalCharIndex / baseTextHandler->numCharsPerTextureRow();
			...
			int x = (int)(charXIndex  * (texDim_ + texPadding_));
			int y = (int)(charYIndex  * (texDim_ + texPadding_));
			int baseX = (int)(baseCharXIndex  * (baseTextHandler->texDim() + baseTextHandler->texPadding()));
			int baseY = (int)(baseCharYIndex  * (baseTextHandler->texDim() + baseTextHandler->texPadding()));

			for (int j = y; j < y+charData.texHeight; ++j)
			{
				for (int i = x; i < x+charData.texWidth; ++i)
				{
					int localX = i - x;
					int localY = j - y;
					buffer[(i+j*textureDim_)]  = pBaseFontBuffer[baseX + localX + (int)baseTextHandler->textureDim()*(baseY + localY)];

				}
			}


			charData.width = pBaseCharData->width;
			charData.height = pBaseCharData->height;
			charData.left = pBaseCharData->left;
			charData.top = pBaseCharData->top;
			charData.advanceX = pBaseCharData->advanceX;
			charData_[c] = charData;
...
// 	else if(glyphIndex == 0 && c != 0x0A){
// 		return;
// 	}


	err = FT_Load_Glyph(ftFace_, glyphIndex,  FT_LOAD_NO_BITMAP | FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH );


	charData.index = index;
	charData.glyphIndex = glyphIndex;	
	
	if (font_.italic && !italicBaked_) 
		FT_GlyphSlot_Oblique(ftFace_->glyph);
	
	if (font_.bold && !boldBaked_)
	{
		ApplyBold(ftFace_->glyph);
...
// 
// 		FT_Stroker stroker;
// 		FT_Stroker_New(Global::instance().ftLibrary(), &stroker);
// 
...
// 		
// 
// 		if (FT_Get_Glyph(ftFace_->glyph, &glyph) == 0)
// 		{
// 			FT_Vector origin;
// 
// 			FT_Glyph_StrokeBorder(&glyph, stroker, 0, 1);
// 
// 			//err = FT_Glyph_To_Bitmap( &glyph, FT_RENDER_MODE_NORMAL, &origin, 0);
// 
...
// 	else
	...
	charData.texWidth = bitmap->width;
	charData.texHeight = bitmap->rows;
	...
	if (bitmap->pixel_mode == FT_PIXEL_MODE_GRAY)
	{
		...
		for (int j = y; j < y+bitmap->rows; ++j)
		{
			for (int i = x; i < x+bitmap->width; ++i)
			{
				...
				if(glyphIndex)
					buffer[(i+j*textureDim_)]  = bitmap->buffer[localX + bitmap->pitch*localY];
				else
					buffer[(i+j*textureDim_)]  = 255;
			}
		}
	}
	else 
...
//	FT_Set_Char_Size(ftFace_, font_.pointSize << 6, font_.pointSize << 6, 96, 96);
	FT_Set_Char_Size(ftFace_, font_.pointSize * 64, font_.pointSize * 64, 96, 96);

	FT_Load_Glyph(ftFace_, glyphIndex, FT_LOAD_NO_BITMAP | FT_LOAD_NO_SCALE | FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH );
	...
	//if (font_.italic && !italicBaked_) {
		charData.width = FT_MulFix(bbox.xMax - bbox.xMin, ftFace_->size->metrics.x_scale) * 1.0f / 64.0f;	
		charData.height = FT_MulFix(bbox.yMax - bbox.yMin, ftFace_->size->metrics.y_scale) * 1.0f / 64.0f;
		charData.left = FT_MulFix( bbox.xMin, ftFace_->size->metrics.x_scale ) * 1.0f / 64.0f;
		charData.top = FT_MulFix( bbox.yMax, ftFace_->size->metrics.y_scale ) * 1.0f / 64.0f;

// 	}else{
// 		charData.width = FT_MulFix( ftFace_->glyph->metrics.width, ftFace_->size->metrics.x_scale ) * 1.0f / 64.0f;
// 		charData.height = FT_MulFix( ftFace_->glyph->metrics.height, ftFace_->size->metrics.y_scale ) * 1.0f / 64.0f;
// 		charData.left = FT_MulFix( ftFace_->glyph->metrics.horiBearingX, ftFace_->size->metrics.x_scale ) * 1.0f / 64.0f;
// 		charData.top = FT_MulFix( ftFace_->glyph->metrics.horiBearingY, ftFace_->size->metrics.y_scale ) * 1.0f / 64.0f;
// 	}




	charData.advanceX = FT_MulFix( ftFace_->glyph->advance.x, ftFace_->size->metrics.x_scale ) * 1.0f / 64.0f;
...
void TextHandler::genAnyNewChars(const std::wstring & str){
	if (!init_) return;
	...
	for (int i = 0; i < strLen; ++i)
	{
		...
		if (charData_.find(c) == charData_.end()) newChars.insert(c);
	}

	genChars(newChars);
}

void TextHandler::genAnyNewChars(
...
void TextHandler::genChars(const std::set<wchar_t> & chars)
{
	//determine the number of texture sheets needed
	int numChars = (int)chars.size();
	if (numChars == 0) return;
	if(numCharsPerTexture_ == 0) return;
...
// 			if(Global::instance().curSceneMagnification() > 1.0){
...
void TextHandler::addChars(const std::string & str)
{
...
StringData * TextHandler::genString(
	GfxRenderer * gl,
	const std::wstring & str, 
	int direction,
	const std::vector<TextProperties_2> & properties,
	...
	float width, float height, float letterSpacing)
{	
	if (!init_) return 0;
	...
	int curLineStyleIndex;
	int nextLineStyleCharIndex;

	int curLineIndex;
	int nextLineIndex;

	TextHandler * curPropTextHandler;
	bool doCurProp;

	std::vector<VertexStruct> vertices;
	std::vector<GLushort> indices;

	StringData * newStr = new StringData;
		...
		++curTextureIndex)
	{
		...
		WordWrapPoint_2 *curLine = (WordWrapPoint_2 *)&lines[0];
		LineStyle *curLineStyle = (LineStyle *)&linesStyle[0];
		TextProperties_2 *curProp = (TextProperties_2 *)&properties[0];

		if(direction == Text_2::DLeft)
			curX = width;

		baseX = curX - curLine->nStartx; 

		for (int i = 0; i < strLen; ++i)
		{
			ObjectProperties *pTargetOP = NULL;
			...
			if (i == nextPropsCharIndex)
			{
				...
				curProp = (TextProperties_2 *)&properties[curPropsIndex];
				curPropTextHandler = curProp->textHandler;
				doCurProp = curPropTextHandler == this && 
					properties[curPropsIndex].color == thisProps.color &&
					properties[curPropsIndex].underline_ == thisProps.underline_  &&
					properties[curPropsIndex].cancleline_ == thisProps.cancleline_  &&
					properties[curPropsIndex].letterwidthratio_ == thisProps.letterwidthratio_;
				...
				{
					curSpan.index = i;
					curSpan.cancleline = properties[curPropsIndex].cancleline_ ;
					curSpan.underline = properties[curPropsIndex].underline_ ;
					curSpan.lineIndex = curLineIndex;
					curSpan.texture = curProp->textHandler;
					curSpan.color = properties[curPropsIndex].color ;
					curSpan.x = curX;					
					curSpan.y = curLineY + curLine->ascenderHeight;
					curSpan.width = 0;
			...
			if (i == nextLineStyleCharIndex)
			{
				++curLineStyleIndex;
				if (curLineStyleIndex < (int)linesStyle.size() - 1)
					nextLineStyleCharIndex = linesStyle[curLineStyleIndex + 1].charIndex;
				else
					nextLineStyleCharIndex = -1;


				curLineStyle = (LineStyle *)&linesStyle[curLineStyleIndex];
			}

			if (i == nextLineIndex)
			{
				if(i != 0)
					curLineY += curLine->toNextLine;

				++curLineIndex;

				if (curLineIndex < (int)lines.size() - 1)
					nextLineIndex = lines[curLineIndex + 1].wrapAtIndex;
				else
					nextLineIndex = -1;


				curLine = (WordWrapPoint_2 *)&lines[curLineIndex];				

				newStr->size.y = curLineY + curLine->ascenderHeight - descender_;

				previousIndex = 0;

				lineWidth = curLine->wrapWidth;

				if(curLineIndex != lines.size() - 1)
					lineWidth -= ENTER_SIZE;

				if(curLineStyle->textAlign_ == LineStyle::Left || curLineStyle->textAlign_ == LineStyle::Justify_Left){
					if(direction == Text_2::DRight)
						curX = curLine->nStartx;
					else if(direction == Text_2::DLeft)
						curX = lineWidth - curLine->nStartx;

				}else if(curLineStyle->textAlign_ == LineStyle::Center){

					if(direction == Text_2::DRight)
						curX = width / 2 - lineWidth / 2 + curLine->nStartx;
					else if(direction == Text_2::DLeft)
						curX = width / 2 + lineWidth / 2 - curLine->nStartx;
					...
					if(direction == Text_2::DRight)
						curX = width - lineWidth + curLine->nStartx;
					else if(direction == Text_2::DLeft)
						curX = width - curLine->nStartx;
				}
				...
				if (constructingSpan)
				{
					...
					curSpan.texture = curProp->textHandler;
					curSpan.x = curX;
					curSpan.y = curLineY + lines[curLineIndex].ascenderHeight;
					curSpan.width = 0;
			...
			bool isIgnored = c == L'\t' || c == L'\r' || c == L'\b' || c == L'\n' || c == IMAGE_OBJECT_STRING;
			//if (c == L'\t' || c == L'\r' || c == L'\b' || c == L'\n') continue;			
			bool isTap = c == L'\t';
			
			CharData & data = curPropTextHandler->charData_[c];

			

			if(curProp->isObjectProperty()){
				pTargetOP = (ObjectProperties *)curProp->GetObjectProperty();

				data.height = data.texHeight = pTargetOP->height;
				data.width = data.texWidth = pTargetOP->width;
				data.advanceX = data.width;
				data.left = 0;
				data.top = 0;
				pTargetOP->x =  curX;

				pTargetOP->y = curLineY;
			...
			if(isTap){
				...
				nTabCount = abs(curX - baseX)/ TAB_SIZE;

				if(direction == Text_2::DRight){
					nModule = fmod((double)curX ,TAB_SIZE);
				}else{
					nModule = fmod((double)(width - curX) ,TAB_SIZE);
				}

				if(nModule >= 0.0)
					nTabCount ++;

				data.width = (nTabCount * TAB_SIZE) - abs(curX - baseX);
				data.advanceX = data.width;
				data.left = 0;
			}
			...
			if (doCurProp) 
			{
				...
				if(i == curLineStyle->charIndex && curLineStyle->lineIncreasement_.type != NONE){

				
					float indexX = baseX + curLineStyle->tabIndex_ * MAX_INCREASEMENT_SIZE;

					BOOST_FOREACH(const wchar_t &indexC, curLineStyle->lineIncreasement_.wstrIndex){
					
						CharData & indexData = curLineStyle->lineIncreasement_.prop.textHandler->charData_[indexC];

						float curY = curLineY + lines[curLineIndex].ascenderHeight -  indexData.top;
						int textureIndex = indexData.index / numCharsPerTexture_;

						int charIndex = indexData.index - textureIndex * numCharsPerTexture_;

						int charXIndex = charIndex % numCharsPerTextureRow_;
						int charYIndex = charIndex / numCharsPerTextureRow_;

						float texX = (float)(charXIndex * (texDim_ + texPadding_)) / (float)textureDim_;
						float texY = (float)(charYIndex * (texDim_ + texPadding_)) / (float)textureDim_;		
						float dx = (indexData.width);
						float dy = (indexData.height);
						//float texDx = (float)dx / textureDim_;
						//float texDy = (float)dy / textureDim_;
						float texDx = (float)(indexData.texWidth) / (float)textureDim_;
						float texDy = (float)(indexData.texHeight) / (float)textureDim_;
			...
			if (doCurProp) 
			{
				...
				float texX = (float)(charXIndex * (texDim_ + texPadding_)) / (float)textureDim_;
				float texY = (float)(charYIndex * (texDim_ + texPadding_)) / (float)textureDim_;		
				float dx = (data.width * curProp->letterwidthratio_ / 100.0f);
				float dy = (data.height);
				//float texDx = (float)dx / textureDim_;
				//float texDy = (float)dy / textureDim_;
				float texDx = (data.texWidth) / (float)textureDim_;
				float texDy = (data.texHeight) / (float)textureDim_;
			...
			int lineStartIndex = curLine->wrapAtIndex;//시작 텍스트
			float linelen = curLine->wrapWidth;//텍스트 라인 길이
			int lineEndIndex = strLen;//마지막 텍스트
			
			if(curLine != &lines[lines.size()-1])
				lineEndIndex = lines[curLineIndex+1].wrapAtIndex - 1;

			int linestyleEndIndex = strLen;

			if(curLineStyle != &linesStyle[linesStyle.size()-1])
				linestyleEndIndex = linesStyle[curLineStyleIndex+1].charIndex - 1;

			float justifySpacing = (width - linelen) / (lineEndIndex - lineStartIndex - 1);
			
			bool JustifyCheck = curLineStyle->textAlign_ == LineStyle::Justify_Left;
				...
				if(isTap || curProp->isObjectProperty()){
					...
					if(direction == Text_2::DRight)
					{
						if(JustifyCheck && lineEndIndex != linestyleEndIndex)
							curX += (data.advanceX + letterSpacing + justifySpacing);
						else 
							curX += (data.advanceX  + letterSpacing);
					}
					else if(direction == Text_2::DLeft)
					{
						if(JustifyCheck && lineEndIndex != linestyleEndIndex)
					...
					{
						if(JustifyCheck && lineEndIndex != linestyleEndIndex)
							curX += (data.advanceX * curProp->letterwidthratio_ / 100.0f + letterSpacing + justifySpacing);
						else
							curX += (data.advanceX * curProp->letterwidthratio_ / 100.0f + letterSpacing);
					}
					else if(direction == Text_2::DLeft)
					{
						if(JustifyCheck && lineEndIndex != linestyleEndIndex)
...
void TextHandler::resizeText()
{
	//charData_.clear();
	//init();
}

bool TextHandler::asyncLoadUpdate()
{
